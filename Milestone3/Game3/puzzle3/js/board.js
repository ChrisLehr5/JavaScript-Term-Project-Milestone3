...
Board.CELL_WIDTH = 360;
Board.CELL_PADDING = 40;
Board.GRID_WIDTH = 3;
Board.GRID_HEIGHT = 3;
...
Board.prototype.triggerCell = function(x, y) {
    this.toggleOnOff(x, y);
    this.toggleOnOff(x, y - 1);
    this.toggleOnOff(x + 1, y);
    this.toggleOnOff(x, y + 1);
    this.toggleOnOff(x - 1, y);
}

Board.prototype.toggleOnOff = function(x, y) {
...
Board.prototype.getCellFromPosition = function(x, y) {
...
Board.prototype.show = function() {
    ...
    for (i = 0; i < Board.GRID_WIDTH; i++) {
        for (j = 0; j < Board.GRID_HEIGHT; j++) {
            this.art.drawCellForm(i, j, this.cells[i][j]);
        }
    }
};